...
# Número de frames entre detecciones (por defecto 25). Cambia este valor para ajustar la frecuencia.
DETECT_EVERY = 10
...
async def videoReceiver():
    ...
    try:
        # intentar conectar al signaling; si falla, caemos a modo webcam local
        try:
            await signaling.connect()
        except Exception as e:
            print(f"No se pudo conectar al signaling {IP_server}:9999 -> {e}. Iniciando fallback local (webcam).")
            # ejecutamos el bucle de webcam en un thread del executor para no bloquear el event loop
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, run_local_webcam, video_receiver)
            return
...
def run_local_webcam(video_receiver):
    """Fallback sin WebRTC: usa la webcam local a través de Detector y muestra frames con OpenCV."""
    det = video_receiver.detector
    cap = det.cap
    if cap is None or not cap.isOpened():
        print('No hay webcam disponible para el fallback local.')
        return
    ...
    try:
        while True:
            ...
            if not ret:
                break
            frame_count += 1
            if video_receiver.objectID and frame_count % DETECT_EVERY == 0:
                try:
                    detectado, rectangulo = det.detect(frame, video_receiver.objectID)
                except Exception as ex:
                    print(f'Error en la detección local: {ex}')
                    detectado = False
...
def crear_ventana():
    ...
    # Botones añadidos: Pastel, Gato, Cuchillo (fila siguiente)
    pastelBtn = tk.Button(detectFrame, text="Pastel", bg="dark orange", command=pastel)
    pastelBtn.grid(row=1, column=0, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)

    gatoBtn = tk.Button(detectFrame, text="Gato", bg="dark orange", command=gato)
    gatoBtn.grid(row=1, column=1, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)

    perroBtn = tk.Button(detectFrame, text="Perro", bg="dark orange", command=perro)
    perroBtn.grid(row=1, column=2, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
