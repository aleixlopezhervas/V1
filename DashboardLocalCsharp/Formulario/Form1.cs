...
namespace Formulario
{
    ...
    {
        Dron dron = new Dron();
        // Última posición conocida (actualizada desde la telemetría)
        ...
        private void but_takeoff_Click(object sender, EventArgs e)
        {
            ...
            int valorSeleccionado = takeoffTrackBar.Value;
            dron.Despegar(valorSeleccionado, bloquear: false, EnAire, "Volando");
            despegarBtn.BackColor = Color.Yellow;
        }

        // Barra despegue
        private void takeoffTrackBar_Scroll(object sender, EventArgs e)
        {
            // Recojo el valor del heading seleccionado
            int n = takeoffTrackBar.Value;
            takeoffLabel.Text = n.ToString();
        }
        ...
        private void ProcesarTelemetria(byte id, List<(string nombre, float valor)> telemetria)
        {
            ...
            // Guardar posición actual para usarla desde la UI (ej. slider de altitud)
            currentLat = lat;
            currentLon = lon;
        ...
        private void altitudTrackBar_Scroll(object sender, EventArgs e)
        {
            // Mostrar el valor seleccionado en la etiqueta mientras se mueve la barra
            int valorSeleccionado = altitudTrackBar.Value;
            setAltitudLbl.Text = valorSeleccionado.ToString();
        }

        private void altitudTrackBar_MouseUp(object sender, MouseEventArgs e)
        {
            // Al soltar la barra, enviar el dron al punto actual (lat, lon)
            // usando la altitud seleccionada en la barra.
            int valorSeleccionado = altitudTrackBar.Value;

            if (double.IsNaN(currentLat) || double.IsNaN(currentLon))
            {
                // no tenemos posición válida todavía
                return;
            }

            // Llamada a la librería para ir al punto con la altitud seleccionada
            dron.IrAlPunto((float)currentLat, (float)currentLon, valorSeleccionado);
        }
    }
}
