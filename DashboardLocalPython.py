...
def showTelemetryInfo (telemetry_info):
    ...
    global altShowLbl, headingShowLbl, stateShowLbl, groundSpeedShowLbl
    ...
    # mostramos la velocidad sobre el suelo si viene en la telemetría
    try:
        groundSpeedShowLbl['text'] = round(telemetry_info['groundSpeed'],2)
    except Exception:
        groundSpeedShowLbl['text'] = ''
...
def onGround(*args):
    # callback cuando ha terminado el aterrizaje
    landBtn['text'] = 'En tierra'
    landBtn['fg'] = 'white'
    landBtn['bg'] = 'green'
    # restaurar el boton de despegar a su estado original
...
def atHome(*args):
    # callback cuando ha terminado el RTL
    RTLBtn['text'] = 'En casa'
    RTLBtn['fg'] = 'white'
    RTLBtn['bg'] = 'green'
    # restaurar el boton de despegar a su estado original
...
def takeoff ():
    global dron
    # despegamos a la altura seleccionada en el slider (no bloqueante)
    altitude_to = int(takeoffHeightSldr.get())
    dron.takeOff (altitude_to, blocking = False,  callback = inTheAir)
    takeOffBtn['text'] = 'Despegando...'
    takeOffBtn['fg'] = 'black'
    takeOffBtn['bg'] = 'yellow'
    # restaurar botones RTL y aterrizar a su estado original cuando se inicie el despegue
    try:
...
def land ():
    global dron
    # llamada no bloqueante con callback onGround
    landBtn['text'] = 'Aterrizando...'
    landBtn['fg'] = 'black'
    landBtn['bg'] = 'yellow'
    dron.Land(blocking=False, callback=onGround)

def RTL():
    global dron
    # llamada no bloqueante con callback atHome
    RTLBtn['text'] = 'Volviendo...'
    RTLBtn['fg'] = 'black'
    RTLBtn['bg'] = 'yellow'
    dron.RTL(blocking=False, callback=atHome)
...
def changeHeading (event):
    ...
    deg = int(gradesSldr.get())
    # si el slider está en 0, mapear a 360 para evitar que params sea falsy
    if deg == 0:
        deg = 360
    dron.changeHeading(deg)
...
def changeAltitude(event):
    global dron
    # cambiamos la altitud del dron mientras vuela (no bloqueante)
    try:
        alt = int(altSldr.get())
        dron.change_altitude(alt, blocking=False)
    except Exception:
        pass
...
def crear_ventana():
    global dron
    global  altShowLbl, headingShowLbl,  speedSldr, gradesSldr, stateShowLbl, groundSpeedShowLbl
    global connectBtn, armBtn, takeOffBtn, landBtn, RTLBtn
    global previousBtn, takeoffHeightSldr, altSldr # aqui guardaré el ultimo boton de navegación clicado
    ...
    # Slider para seleccionar la altura de takeoff (encima del slider de grados)
    takeoffHeightSldr = tk.Scale(ventana, label="Altura takeoff (m):", resolution=1, from_=1, to=30, tickinterval=5,
                          orient=tk.HORIZONTAL)
    takeoffHeightSldr.set(1)
    takeoffHeightSldr.grid(row=3, column=0, columnspan=2,padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)

    # Slider para seleccionar el heading (se ha movido una fila abajo)
    ...
    # slider para elegir la altitud del dron mientras está volando (encima del slider de velocidad)
    altSldr = tk.Scale(ventana, label="Altitud (m):", resolution=1, from_=1, to=20, tickinterval=5,
                         orient=tk.HORIZONTAL)
    altSldr.set(1)
    altSldr.grid(row=7, column=0, columnspan=2, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
    altSldr.bind("<ButtonRelease-1>", changeAltitude)

    # slider para elegir la velocidad de navegación (se ha movido una fila abajo)
    ...
    groundSpeedLbl = tk.Label(telemetryFrame, text='groundSpeed')
    groundSpeedLbl.grid(row=0, column=3,  padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
    ...
    groundSpeedShowLbl = tk.Label(telemetryFrame, text='', )
    groundSpeedShowLbl.grid(row=1, column=3, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
